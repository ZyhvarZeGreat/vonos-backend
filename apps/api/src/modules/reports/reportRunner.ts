...
export async function runReportForTenant(
  ...
  switch (source.kind) {
    ...
    case 'payment-accounts': {
      ...
      if (handler === 'balance-sheet') return buildBalanceSheetReport(db, from, to);
      if (handler === 'trial-balance') return buildTrialBalanceReport(db, from, to);
      if (handler === 'cash-flow') return buildCashFlowReport(db, from, to);
      return buildPaymentAccountReport(db, from, to);
    }
    case 'reports':
      if (archetype === 'stock') {
        return buildStockReports(db, source.tab as 'valuation' | 'movement' | 'lowstock', from, to);
      }
      return buildTransactionReports(db, source.tab as 'sales' | 'closeout', from, to);
    case 'stock':
    ...
    case 'product': {
      const handler = source.handler;
      if (handler === 'trending') return buildTrendingProductsReport(db, from, to);
      if (handler === 'items') return buildItemsReport(db, from, to);
      if (handler === 'purchase') return buildProductPurchaseReport(db, from, to);
      return buildProductSellReport(db, from, to);
    }
    case 'sales': {
      const handler = source.handler;
      if (handler === 'purchase-sale') return buildPurchaseSaleReport(db, from, to);
    ...
    case 'audit': {
      const logs = await prisma.auditLog.findMany({
        where: { tenantId, ...(from || to ? { occurredAt: { ...(from ? { gte: new Date(from) } : {}), ...(to ? { lte: new Date(to) } : {}) } } : {}) },
        orderBy: { occurredAt: 'desc' },
        take: 200,
      });
      return {
        kpis: [{ label: 'Log entries', icon: 'activity', metricKey: 'logs', color: '#2563eb', value: logs.length }],
        charts: [],
        table: {
          columns: [
...
export async function runGroupReport(
  ...
  from?: string,
  to?: string,
): Promise<ReportsDashboard & { byEntity?: Array<{ code: string; rows: Record<string, string | number>[] }> }> {
  const entry = reportEntryById(reportId);
  ...
  const byEntity: Array<{ code: string; rows: Record<string, string | number>[] }> = [];
