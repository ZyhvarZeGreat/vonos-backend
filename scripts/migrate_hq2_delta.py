#!/usr/bin/env python3
"""Backfill ETL: vonomglk_hq2 (2025) → Vonos Automotive (tenant_va_001)."""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Migrate VA hq2 archive (2025 only) from localhost.sql",
    )
    parser.add_argument(
        "--dump",
        type=Path,
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("docs/migration-audits/dryruns/HQ2_DELTA_DRYRUN.json"),
    )
    ...
    progress.message("\nVA hq2 2025 backfill migration")
