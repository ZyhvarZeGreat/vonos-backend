# VW_TEMP — MySQL Dump Audit

**Source database:** `vonomglk_hq3temp`
**Business name (from `business` table):** Vonos Autos HQ
**Dump file:** `localhost.sql` (lines 226,197–303,082)
**Generated:** audit script run against cPanel/phpMyAdmin export

---

## 1. Table Inventory

**Total tables:** 97 | **Populated:** 50 | **Empty:** 47

### All tables (with row counts)

| Table | Rows | Status |
|---|---:|---|
| `activity_log` | 14,093 | populated |
| `transaction_sell_lines` | 6,578 | populated |
| `transaction_sell_lines_purchase_lines` | 6,485 | populated |
| `account_transactions` | 6,223 | populated |
| `transaction_payments` | 6,219 | populated |
| `transactions` | 5,680 | populated |
| `contacts` | 5,434 | populated |
| `purchase_lines` | 3,337 | populated |
| `product_variations` | 3,078 | populated |
| `variations` | 3,078 | populated |
| `products` | 2,979 | populated |
| `product_locations` | 2,757 | populated |
| `product_racks` | 2,738 | populated |
| `variation_location_details` | 1,229 | populated |
| `role_has_permissions` | 573 | populated |
| `essentials_payroll_group_transactions` | 364 | populated |
| `migrations` | 352 | populated |
| `model_has_permissions` | 263 | populated |
| `model_has_roles` | 211 | populated |
| `users` | 200 | populated |
| `permissions` | 178 | populated |
| `categories` | 168 | populated |
| `currencies` | 141 | populated |
| `expense_categories` | 48 | populated |
| `accounts` | 47 | populated |
| `essentials_payroll_groups` | 43 | populated |
| `essentials_user_allowance_and_deductions` | 31 | populated |
| `brands` | 30 | populated |
| `roles` | 28 | populated |
| `system` | 25 | populated |
| `reference_counts` | 13 | populated |
| `notification_templates` | 9 | populated |
| `media` | 6 | populated |
| `cash_registers` | 3 | populated |
| `units` | 3 | populated |
| `user_contact_access` | 3 | populated |
| `account_types` | 2 | populated |
| `business_locations` | 2 | populated |
...
- `essentials_payroll_group_transactions` (364 rows)
- `essentials_payroll_groups` (43 rows)
...
- `essentials_user_allowance_and_deductions` (31 rows)
...
**qty_available:** non-zero=1,146, zero=83, NULL=0
...
**Location count:** 2 business location(s) — multi-location
...
- **Transaction date range:** 2026-01-01 → 2026-06-18 (5,680 dated rows sampled)
- **Operational history:** 5,680 transactions — appears to be real operational data
- **Test products:** None detected in product name sample
- **Stock quantities (`variation_location_details`):** 1,146 non-zero, 83 zero, 0 NULL (93.2% populated with stock)
...
|---|---|---|
| `purchase` | StockMovement(inbound) | 2,411 rows; StockMovement(inbound) + LedgerEntry(cost) + supplier link |
| `sell` | Order/Sale | 1,532 rows; Order/Sale + LedgerEntry(revenue) + stock decrement |
| `expense` | LedgerEntry(expense) | 1,370 rows; LedgerEntry(expense) |
| `payroll` | LedgerEntry(expense) — payroll category | 358 rows; LedgerEntry(expense) — payroll category |
| `opening_stock` | Seed Item.quantity via variation_location_details | 9 rows; Seed Item.quantity via variation_location_details |
