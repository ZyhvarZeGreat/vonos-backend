...
| Table | Rows | Status |
|---|---:|---|
| `activity_log` | 13,739 | populated |
| `transaction_sell_lines` | 6,327 | populated |
| `transaction_sell_lines_purchase_lines` | 6,279 | populated |
| `account_transactions` | 6,068 | populated |
| `transaction_payments` | 6,062 | populated |
| `transactions` | 5,526 | populated |
| `contacts` | 5,419 | populated |
| `purchase_lines` | 3,265 | populated |
| `product_variations` | 3,073 | populated |
| `variations` | 3,073 | populated |
| `products` | 2,974 | populated |
| `product_locations` | 2,752 | populated |
| `product_racks` | 2,738 | populated |
| `variation_location_details` | 1,211 | populated |
| `role_has_permissions` | 573 | populated |
| `essentials_payroll_group_transactions` | 364 | populated |
| `migrations` | 352 | populated |
| `model_has_permissions` | 262 | populated |
| `model_has_roles` | 210 | populated |
| `users` | 199 | populated |
...
**qty_available:** non-zero=1,129, zero=82, NULL=0
...
- **Transaction date range:** 2026-01-01 → 2026-06-13 (5,526 dated rows sampled)
- **Operational history:** 5,526 transactions — appears to be real operational data
- **Test products:** None detected in product name sample
- **Stock quantities (`variation_location_details`):** 1,129 non-zero, 82 zero, 0 NULL (93.2% populated with stock)
...
|---|---|---|
| `purchase` | StockMovement(inbound) | 2,358 rows; StockMovement(inbound) + LedgerEntry(cost) + supplier link |
| `sell` | Order/Sale | 1,472 rows; Order/Sale + LedgerEntry(revenue) + stock decrement |
| `expense` | LedgerEntry(expense) | 1,330 rows; LedgerEntry(expense) |
| `payroll` | LedgerEntry(expense) — payroll category | 358 rows; LedgerEntry(expense) — payroll category |
| `opening_stock` | Seed Item.quantity via variation_location_details | 8 rows; Seed Item.quantity via variation_location_details |
