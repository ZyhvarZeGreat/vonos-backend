# VW_OLD — MySQL Dump Audit

**Source database:** `vonomglk_OLD_hq2`
**Business name (from `business` table):** Vonos Autos HQ
**Dump file:** `localhost.sql` (lines 303,083–419,611)
...
| Table | Rows | Status |
|---|---:|---|
| `activity_log` | 27,580 | populated |
| `transaction_sell_lines` | 14,171 | populated |
| `transaction_sell_lines_purchase_lines` | 13,875 | populated |
| `transaction_payments` | 10,493 | populated |
| `account_transactions` | 10,390 | populated |
| `transactions` | 9,829 | populated |
| `purchase_lines` | 4,657 | populated |
| `contacts` | 4,602 | populated |
| `product_locations` | 2,359 | populated |
| `product_variations` | 2,359 | populated |
| `variations` | 2,359 | populated |
| `products` | 2,261 | populated |
| `variation_location_details` | 2,198 | populated |
| `product_racks` | 1,934 | populated |
| `essentials_payroll_group_transactions` | 902 | populated |
| `role_has_permissions` | 463 | populated |
| `migrations` | 352 | populated |
| `model_has_permissions` | 206 | populated |
| `permissions` | 169 | populated |
| `categories` | 166 | populated |
| `model_has_roles` | 153 | populated |
| `users` | 142 | populated |
| `currencies` | 141 | populated |
| `essentials_payroll_groups` | 54 | populated |
| `expense_categories` | 48 | populated |
| `notifications` | 42 | populated |
| `accounts` | 38 | populated |
| `brands` | 30 | populated |
| `transactions_main` | 27 | populated |
| `system` | 25 | populated |
| `roles` | 19 | populated |
| `reference_counts` | 11 | populated |
...
- `essentials_payroll_group_transactions` (902 rows)
- `essentials_payroll_groups` (54 rows)
...
**qty_available:** non-zero=1,438, zero=760, NULL=0
...
- **Transaction date range:** 2025-01-01 → 2025-09-16 (9,829 dated rows sampled)
- **Operational history:** 9,829 transactions — appears to be real operational data
- **Test products:** None detected in product name sample
- **Stock quantities (`variation_location_details`):** 1,438 non-zero, 760 zero, 0 NULL (65.4% populated with stock)
...
|---|---|---|
| `sell` | Order/Sale | 3,848 rows; Order/Sale + LedgerEntry(revenue) + stock decrement |
| `purchase` | StockMovement(inbound) | 3,395 rows; StockMovement(inbound) + LedgerEntry(cost) + supplier link |
| `expense` | LedgerEntry(expense) | 1,871 rows; LedgerEntry(expense) |
| `payroll` | LedgerEntry(expense) — payroll category | 451 rows; LedgerEntry(expense) — payroll category |
| `opening_stock` | Seed Item.quantity via variation_location_details | 263 rows; Seed Item.quantity via variation_location_details |
