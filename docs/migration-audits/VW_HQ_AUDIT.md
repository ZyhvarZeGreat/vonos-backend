# VW_HQ — MySQL Dump Audit (legacy archive)

> **Not the Vonos Warehouse cutover source.** Canonical VW uses `vonomglk_audit` — see [VW_AUDIT.md](./VW_AUDIT.md).

**Source database:** `vonomglk_hq2`
**Business name (from `business` table):** Vonos Autos HQ
**Dump file:** `vonomglk_hq2` slice from `localhost (1).sql` (Jun 18, 2026) — **archive only, not VW cutover source**
**Generated:** Jun 24, 2026 audit refresh

---

## 1. Table Inventory

**Total tables:** 97 | **Populated:** 51 | **Empty:** 46

### All tables (with row counts)

| Table | Rows | Status |
|---|---:|---|
| `activity_log` | 40,270 | populated |
| `transaction_sell_lines` | 20,938 | populated |
| `transaction_sell_lines_purchase_lines` | 20,032 | populated |
| `transaction_payments` | 15,615 | populated |
| `account_transactions` | 15,513 | populated |
| `transactions` | 14,817 | populated |
| `purchase_lines` | 7,637 | populated |
| `contacts` | 4,999 | populated |
| `product_locations` | 2,436 | populated |
| `product_variations` | 2,436 | populated |
| `variations` | 2,436 | populated |
| `products` | 2,337 | populated |
| `variation_location_details` | 2,279 | populated |
| `product_racks` | 1,934 | populated |
| `essentials_payroll_group_transactions` | 1,210 | populated |
| `role_has_permissions` | 467 | populated |
| `migrations` | 352 | populated |
| `model_has_permissions` | 231 | populated |
| `model_has_roles` | 178 | populated |
| `permissions` | 169 | populated |
| `categories` | 167 | populated |
| `users` | 167 | populated |
| `currencies` | 141 | populated |
| `essentials_payroll_groups` | 85 | populated |
| `expense_categories` | 48 | populated |
| `notifications` | 42 | populated |
| `accounts` | 39 | populated |
| `brands` | 30 | populated |
| `transactions_main` | 27 | populated |
| `system` | 25 | populated |
| `roles` | 19 | populated |
| `reference_counts` | 12 | populated |
| `notification_templates` | 9 | populated |
| `media` | 6 | populated |
| `user_contact_access` | 4 | populated |
| `units` | 3 | populated |
| `account_types` | 2 | populated |
| `cash_registers` | 2 | populated |
| `customer_groups` | 2 | populated |
| `password_resets` | 2 | populated |
| `tax_rates` | 2 | populated |
| `types_of_services` | 2 | populated |
| `barcodes` | 1 | populated |
| `business` | 1 | populated |
| `business_locations` | 1 | populated |
| `cash_register_transactions` | 1 | populated |
| `essentials_attendances` | 1 | populated |
| `essentials_to_dos` | 1 | populated |
| `essentials_todos_users` | 1 | populated |
...
| `essentials_allowances_and_deductions` | 0 | empty |
| `essentials_document_shares` | 0 | empty |
| `essentials_documents` | 0 | empty |
| `essentials_holidays` | 0 | empty |
| `essentials_kb` | 0 | empty |
| `essentials_kb_users` | 0 | empty |
| `essentials_leave_types` | 0 | empty |
| `essentials_leaves` | 0 | empty |
| `essentials_messages` | 0 | empty |
| `essentials_reminders` | 0 | empty |
| `essentials_shifts` | 0 | empty |
| `essentials_todo_comments` | 0 | empty |
| `essentials_user_allowance_and_deductions` | 0 | empty |
| `essentials_user_sales_targets` | 0 | empty |
| `essentials_user_shifts` | 0 | empty |
...
| `sessions` | 0 | empty |
| `stock_adjustment_lines` | 0 | empty |
...
| `warranties` | 0 | empty |
| `woocommerce_sync_logs` | 0 | empty |

---

## 2. Schema Comparison

**Verdict:** Ultimate POS base + 27 extension table(s)

- Core Ultimate POS tables present: **70 / 77**
- Extension/extra tables: **27**
- Missing from base 77: **0**

### Extra tables (not in base Ultimate POS 77)

- `essentials_allowances_and_deductions` (0 rows)
- `essentials_attendances` (1 rows)
- `essentials_document_shares` (0 rows)
- `essentials_documents` (0 rows)
- `essentials_holidays` (0 rows)
- `essentials_kb` (0 rows)
- `essentials_kb_users` (0 rows)
- `essentials_leave_types` (0 rows)
- `essentials_leaves` (0 rows)
- `essentials_messages` (0 rows)
- `essentials_payroll_group_transactions` (1,210 rows)
- `essentials_payroll_groups` (85 rows)
- `essentials_reminders` (0 rows)
- `essentials_shifts` (0 rows)
- `essentials_to_dos` (1 rows)
- `essentials_todo_comments` (0 rows)
- `essentials_todos_users` (1 rows)
- `essentials_user_allowance_and_deductions` (0 rows)
- `essentials_user_sales_targets` (0 rows)
- `essentials_user_shifts` (0 rows)
- `packages` (0 rows)
- `subscriptions` (0 rows)
- `superadmin_communicator_logs` (0 rows)
- `superadmin_frontend_pages` (0 rows)
- `tables` (0 rows)
- `transactions_main` (27 rows)
- `woocommerce_sync_logs` (0 rows)
...
| `essentials_amount_per_unit_duration` | decimal(22,4) NOT NULL |
| `essentials_allowances` | text DEFAULT NULL |
| `essentials_deductions` | text NOT NULL |
...
**qty_available:** non-zero=1,631, zero=648, NULL=0
...
- **Transaction date range:** 2025-01-01 → 2026-12-01 (14,817 dated rows sampled)
- **Operational history:** 14,817 transactions — appears to be real operational data
- **Test products:** None detected in product name sample
- **Stock quantities (`variation_location_details`):** 1,631 non-zero, 648 zero, 0 NULL (71.6% populated with stock)
...
|---|---|---|
| `purchase` | StockMovement(inbound) | 5,391 rows; StockMovement(inbound) + LedgerEntry(cost) + supplier link |
| `sell` | Order/Sale | 5,332 rows; Order/Sale + LedgerEntry(revenue) + stock decrement |
| `expense` | LedgerEntry(expense) | 2,740 rows; LedgerEntry(expense) |
| `payroll` | LedgerEntry(expense) — payroll category | 759 rows; LedgerEntry(expense) — payroll category |
| `opening_stock` | Seed Item.quantity via variation_location_details | 593 rows; Seed Item.quantity via variation_location_details |
| `purchase_return` | StockMovement(outbound) | 2 rows; StockMovement(outbound) + LedgerEntry adjustment |
