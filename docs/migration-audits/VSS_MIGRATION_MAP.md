# VSS — Field-Level Migration Map

**Target entity:** Vonos Spare Shop (`VSS`, tenant code `VSS`, seed id `tenant_vss_001`)  
**Canonical source:** `vonomglk_vsp` in `localhost (1).sql` (full cPanel export, Jun 18 2026)  
**Architecture reference:** [VISP_LEGACY_ARCHITECTURE.md](./VISP_LEGACY_ARCHITECTURE.md) — how Ultimate POS at `visp.vonosautomarket.com` structured data and backend flows  
**Audit reference:** [VSP_AUDIT.md](./VSP_AUDIT.md)  
**Not used:** `vonomglk_spmarket` (partial copy — same business name, far fewer rows; see architecture doc §7)
...
| `sell_return` | Not present in VSP (`transactions.type` has only `sell` + `opening_stock`) |
| `woocommerce_*` | No WooCommerce rows in VSP |
| `purchase`, `expense`, `payroll` | No such transaction types in VSP |
...
1. Ensure `Tenant` row exists (`code: VSS`, `archetype: transaction`).
...
| Field | Value |
|---|---|
| `tenantId` | Vonos tenant cuid for VSS |
...
| — | `currency` | `"NGN"` |
| — | `availableForRetail` | `true` for all VSS catalog items |
| — | `status` | See §6 |
| `variations.id` | `MigrationLegacyId` | `entityType: item` |

**Expected count:** ~2,639 variations / ~2,337 with location qty rows (per VSP audit).
...
|---|---|---|
| `invoice_no` | `reference` | Prefer `invoice_no`; fallback `ref_no`; fallback `VSS-{id}` |
...
**Expected count:** 18,474 total sell lines in VSP; subset linked to final sell transactions (~lower after parent filter).
...
Compare dry-run / import counts against [VSP_AUDIT.md](./VSP_AUDIT.md):

| Metric | Expected (VSP) |
|---|---:|
| `products` | 2,540 |
| `variations` | 2,639 |
| `variation_location_details` | 2,337 |
| `contacts` | 4,799 |
| `transactions` (sell) | 3,016 |
| `transactions` (opening_stock) | 2,334 |
| `transaction_sell_lines` | 18,474 |
...
Run: `python scripts/migrate_vss_from_vsp.py --dump localhost.sql --dry-run`

---

## 13. Open questions

1. **VW catalog sync vs local import:** AGENTS.md defers live Spare Shop catalog as filtered query against Warehouse `items`. For cutover, import VSP products as standalone VSS `Item` rows with `availableForRetail: true`. Re-link to VW stock post-migration when cross-tenant requisition flow is designed.

2. **`vonomglk_spmarket`:** Smaller DB (131 sales, 2026-only). Treat as experimental/staging unless team confirms production cutover from spmarket instead of vsp.

3. **Returns:** No `sell_return` in VSP. `SaleReturn` model deferred; Prisma `SaleStatus.refunded` reserved for future.
...
- ETL stub: [`scripts/migrate_vss_from_vsp.py`](../../scripts/migrate_vss_from_vsp.py)
