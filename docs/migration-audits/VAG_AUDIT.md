# VAG — MySQL Dump Audit

**Source database:** `vonomglk_gp`
**Business name (from `business` table):** VONOS GWARIMPA BRANCH
**Dump file:** `localhost.sql` (lines 52,208–61,598)
...
**Total tables:** 90 | **Populated:** 36 | **Empty:** 54

### All tables (with row counts)

| Table | Rows | Status |
|---|---:|---|
| `product_locations` | 883 | populated |
| `product_variations` | 883 | populated |
| `products` | 883 | populated |
| `variations` | 883 | populated |
| `transactions` | 386 | populated |
| `activity_log` | 335 | populated |
| `migrations` | 331 | populated |
| `purchase_lines` | 321 | populated |
| `variation_location_details` | 303 | populated |
| `role_has_permissions` | 294 | populated |
| `permissions` | 169 | populated |
| `model_has_permissions` | 166 | populated |
| `currencies` | 141 | populated |
| `transaction_sell_lines` | 113 | populated |
| `model_has_roles` | 89 | populated |
| `transaction_payments` | 64 | populated |
| `account_transactions` | 57 | populated |
| `transaction_sell_lines_purchase_lines` | 43 | populated |
| `contacts` | 37 | populated |
| `accounts` | 28 | populated |
| `categories` | 14 | populated |
| `roles` | 14 | populated |
| `essentials_payroll_group_transactions` | 11 | populated |
| `users` | 11 | populated |
| `notification_templates` | 10 | populated |
| `barcodes` | 6 | populated |
| `reference_counts` | 6 | populated |
| `essentials_payroll_groups` | 5 | populated |
| `system` | 3 | populated |
| `units` | 3 | populated |
...
**Verdict:** Ultimate POS base + 20 extension table(s)

- Core Ultimate POS tables present: **70 / 77**
- Extension/extra tables: **20**
...
- `essentials_payroll_group_transactions` (11 rows)
- `essentials_payroll_groups` (5 rows)
...
**qty_available:** non-zero=290, zero=13, NULL=0
...
- **Transaction date range:** 2024-01-01 → 2024-08-24 (386 dated rows sampled)
- **Operational history:** 386 transactions — appears to be real operational data
- **Test products:** None detected in product name sample
- **Stock quantities (`variation_location_details`):** 290 non-zero, 13 zero, 0 NULL (95.7% populated with stock)
...
|---|---|---|
| `opening_stock` | Seed Item.quantity via variation_location_details | 321 rows; Seed Item.quantity via variation_location_details |
| `sell` | Order/Sale | 54 rows; Order/Sale + LedgerEntry(revenue) + stock decrement |
| `payroll` | LedgerEntry(expense) — payroll category | 11 rows; LedgerEntry(expense) — payroll category |
