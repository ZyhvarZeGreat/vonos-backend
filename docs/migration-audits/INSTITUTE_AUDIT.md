# INSTITUTE — MySQL Dump Audit

**Source database:** `vonomglk_vonos_institute`
**Business name (from `business` table):** Vonos Automotive Institute
**Dump file:** `localhost.sql` (lines 4,595,494–4,633,957)
...
**Total tables:** 96 | **Populated:** 48 | **Empty:** 48

### All tables (with row counts)

| Table | Rows | Status |
|---|---:|---|
| `contacts` | 5,117 | populated |
| `activity_log` | 4,032 | populated |
| `transaction_sell_lines` | 3,284 | populated |
| `transaction_sell_lines_purchase_lines` | 3,051 | populated |
| `product_locations` | 2,797 | populated |
| `product_variations` | 2,797 | populated |
| `variations` | 2,797 | populated |
| `products` | 2,697 | populated |
| `product_racks` | 1,934 | populated |
| `transactions` | 1,367 | populated |
| `transaction_payments` | 1,349 | populated |
| `account_transactions` | 1,348 | populated |
| `variation_location_details` | 599 | populated |
| `role_has_permissions` | 467 | populated |
| `migrations` | 352 | populated |
| `model_has_permissions` | 210 | populated |
| `permissions` | 169 | populated |
| `categories` | 168 | populated |
| `model_has_roles` | 157 | populated |
| `users` | 146 | populated |
| `currencies` | 141 | populated |
| `purchase_lines` | 113 | populated |
| `expense_categories` | 48 | populated |
| `accounts` | 43 | populated |
| `brands` | 30 | populated |
...
**Verdict:** Ultimate POS base + 26 extension table(s)

- Core Ultimate POS tables present: **70 / 77**
- Extension/extra tables: **26**
...
**qty_available:** non-zero=587, zero=12, NULL=0
...
- **Transaction date range:** 2025-01-01 → 2026-06-04 (1,367 dated rows sampled)
- **Operational history:** 1,367 transactions — appears to be real operational data
- **Test products:** None detected in product name sample
- **Stock quantities (`variation_location_details`):** 587 non-zero, 12 zero, 0 NULL (98.0% populated with stock)
...
|---|---|---|
| `sell` | Order/Sale | 1,253 rows; Order/Sale + LedgerEntry(revenue) + stock decrement |
| `opening_stock` | Seed Item.quantity via variation_location_details | 113 rows; Seed Item.quantity via variation_location_details |
| `expense` | LedgerEntry(expense) | 1 rows; LedgerEntry(expense) |
