...
**Source database:** `vonomglk_Quotation`
**Business name (from `business` table):** Vonos Automotive LTD
**Dump file:** `localhost.sql` (lines 583,939–4,580,689)
...
**Total tables:** 95 | **Populated:** 61 | **Empty:** 34

### All tables (with row counts)

| Table | Rows | Status |
|---|---:|---|
| `transaction_sell_lines_purchase_lines` | 3,821,165 | populated |
| `activity_log` | 47,834 | populated |
| `transaction_sell_lines` | 23,657 | populated |
| `transactions` | 20,616 | populated |
| `transaction_payments` | 16,850 | populated |
| `account_transactions` | 16,693 | populated |
| `purchase_lines` | 8,290 | populated |
| `product_racks` | 7,332 | populated |
| `product_locations` | 5,786 | populated |
| `variation_location_details` | 3,954 | populated |
| `contacts` | 2,690 | populated |
| `product_variations` | 2,366 | populated |
| `variations` | 2,366 | populated |
| `products` | 2,328 | populated |
| `stock_adjustment_lines` | 1,900 | populated |
| `essentials_payroll_group_transactions` | 578 | populated |
| `migrations` | 352 | populated |
| `role_has_permissions` | 168 | populated |
| `permissions` | 166 | populated |
| `model_has_permissions` | 165 | populated |
| `categories` | 153 | populated |
| `currencies` | 141 | populated |
| `expense_categories` | 122 | populated |
| `model_has_roles` | 115 | populated |
| `users` | 99 | populated |
| `woocommerce_sync_logs` | 66 | populated |
| `essentials_payroll_groups` | 47 | populated |
| `notifications` | 42 | populated |
| `brands` | 30 | populated |
| `accounts` | 28 | populated |
| `system` | 25 | populated |
| `reference_counts` | 15 | populated |
| `business_locations` | 13 | populated |
...
- `essentials_payroll_group_transactions` (578 rows)
- `essentials_payroll_groups` (47 rows)
...
| purchase | 2536 |
| sell_transfer | 894 |
| purchase_transfer | 894 |
| stock_adjustment | 601 |
| payroll | 569 |
| purchase_return | 1 |
| expense_refund | 1 |
...
**qty_available:** non-zero=2,888, zero=1,066, NULL=0
...
**Location count:** 13 business location(s) — multi-location
...
- **Transaction date range:** 2022-10-26 → 2024-12-01 (20,616 dated rows sampled)
- **Operational history:** 20,616 transactions — appears to be real operational data
- **Test products:** None detected in product name sample
- **Stock quantities (`variation_location_details`):** 2,888 non-zero, 1,066 zero, 0 NULL (73.0% populated with stock)
...
|---|---|---|
| `sell` | Order/Sale | 6,570 rows; Order/Sale + LedgerEntry(revenue) + stock decrement |
| `expense` | LedgerEntry(expense) | 5,217 rows; LedgerEntry(expense) |
| `opening_stock` | Seed Item.quantity via variation_location_details | 3,333 rows; Seed Item.quantity via variation_location_details |
| `purchase` | StockMovement(inbound) | 2,536 rows; StockMovement(inbound) + LedgerEntry(cost) + supplier link |
| `sell_transfer` | StockMovement(transfer) between locations | 894 rows; StockMovement(transfer) between locations |
| `purchase_transfer` | StockMovement(transfer) inbound side | 894 rows; StockMovement(transfer) inbound side |
| `stock_adjustment` | StockMovement | 601 rows; StockMovement + Item quantity reconcile |
| `payroll` | LedgerEntry(expense) — payroll category | 569 rows; LedgerEntry(expense) — payroll category |
